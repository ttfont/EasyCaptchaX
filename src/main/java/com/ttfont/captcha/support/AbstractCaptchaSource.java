...
import com.ttfont.captcha.utils.FontsUtil;
 ...
 * @Description :
 */
public abstract class AbstractCaptchaSource extends AbstractRandomCharSupport {
    /**
     * 验证码的字体
     */
     ...
     * 验证码类型
     */
    protected int charType = CaptchaConstant.TYPE_DEFAULT;
    ...
    protected Color color() {
        int[] color = CaptchaConstant.COLOR[num(CaptchaConstant.COLOR.length)];
     ...
     * @return
     */
    protected abstract boolean graphicsImage(char[] chars, OutputStream out) throws IllegalAccessException;
    ...
    public Font getFont() {
        if (null == font) {
            try {
                setFont(CaptchaConstant.FONT_1);
            } catch (Exception e) {
    ...
    public void setFont(int font, int style, float size) {
        this.font = FontsUtil.getFont(CaptchaConstant.FONT_NAMES[font], style, size);
    }
